...
func (s *NurseConsumption) Update(w *ecs.World) {
	...
	if s.nurseParams.Nursebeecsv1 {
		...
		if s.stores.Pollen > 0 {
			s.stores.ProteinFactorNurses = util.Clamp(s.stores.ProteinFactorNurses+(s.nurseParams.NurseWorkLoadTH-s.nGlobals.NurseWorkLoad)/s.storeParams.ProteinStoreNurse, 0.0, 1.0)
		} else if s.stores.Pollen <= 0 {
			s.stores.ProteinFactorNurses = util.Clamp(s.stores.ProteinFactorNurses-s.nGlobals.NurseWorkLoad/s.storeParams.ProteinStoreNurse, 0.0, 1.0) // now uses NurseWorkLoad instead of old workLoad which was weirdly dependent on Foragers and thus overall colony size
		}
