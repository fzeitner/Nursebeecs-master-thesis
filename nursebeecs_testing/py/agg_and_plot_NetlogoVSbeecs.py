...
def plot_quantiles(netlogo_file, beecs_file, out_dir, format, appday):
    data_net = pd.read_csv(netlogo_file, sep=r'\s*;\s*', engine='python')
    data_beecs = pd.read_csv(beecs_file, sep=r'\s*;\s*', engine='python')

    columns = list(data_net.columns)[1:]
...
def plot_column(data_net, data_beecs, column, quantiles, image_file, appday):
    median_col = quantiles[len(quantiles) // 2][0]

    fig, ax = plt.subplots(figsize=(10, 4))
    for data, col, model in [
        (data_net, "blue", "BEEHAVE"),
        (data_beecs, "red", "beecs"),
    ]:      
...
if __name__ == "__main__":
    ...
    testfolders = ["default_etox", "default_dimethoate", "default_beecs", "Rothamsted2009_beecs",
                   "Rothamsted2009_fenoxycarb", "Rothamsted2009_etox", "Rothamsted2009_fenoxycarb_5years"]
    folder = testfolders[6]
    ...
    if run_all:
        for folder in testfolders:
            if agg_all:
                ...
                agg_netlogo("nursebeecs_testing/" + folder + "/out/netlogo.csv", "nursebeecs_testing/" + folder + "/netlogo.csv")
            elif agg_net:
                agg_netlogo("nursebeecs_testing/" + folder + "/out/netlogo.csv", "nursebeecs_testing/" + folder + "/netlogo.csv")
            elif agg_beec:
                agg_beecs("nursebeecs_testing/" + folder + "/out/beecs-%04d.csv", "nursebeecs_testing/"+ folder +"/beecs.csv")
            ...
            plot_quantiles(
                "nursebeecs_testing/" + folder + "/netlogo.csv",
